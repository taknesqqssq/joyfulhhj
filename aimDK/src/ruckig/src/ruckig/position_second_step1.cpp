...
PositionSecondOrderStep1::PositionSecondOrderStep1(double p0, double v0,
                                                   double pf, double vf,
                                                   double vMax, double vMin,
                                                   double aMax, double aMin)
    : v0(v0), vf(vf), _vMax(vMax), _vMin(vMin), _aMax(aMax), _aMin(aMin) {
  pd = pf - p0;
}

void PositionSecondOrderStep1::time_acc0(ProfileIter &profile, double vMax,
                                         double vMin, double aMax, double aMin,
                                         bool) const {
  profile->t[0] = (-v0 + vMax) / aMax;
  profile->t[1] = (aMin * v0 * v0 - aMax * vf * vf) / (2 * aMax * aMin * vMax) +
                  vMax * (aMax - aMin) / (2 * aMax * aMin) + pd / vMax;
  ...
  if (profile->check_for_second_order<ControlSigns::UDDU, ReachedLimits::ACC0>(
          aMax, aMin, vMax, vMin)) {
    add_profile(profile);
  }
}

void PositionSecondOrderStep1::time_none(ProfileIter &profile, double vMax,
                                         double vMin, double aMax, double aMin,
                                         bool return_after_found) const {
  double h1 =
      (aMax * vf * vf - aMin * v0 * v0 - 2 * aMax * aMin * pd) / (aMax - aMin);
  if (h1 >= 0.0) {
    h1 = std::sqrt(h1);
    ...
    // Solution 1
    {
      ...
      if (profile
              ->check_for_second_order<ControlSigns::UDDU, ReachedLimits::NONE>(
                  aMax, aMin, vMax, vMin)) {
        add_profile(profile);
...
bool PositionSecondOrderStep1::time_all_single_step(Profile *profile,
                                                    double vMax, double vMin,
                                                    double aMax,
                                                    double aMin) const {
...
bool PositionSecondOrderStep1::get_profile(const Profile &input, Block &block) {
  ...
  if (std::abs(vf) < DBL_EPSILON) {
    // There is no blocked interval when vf==0, so return after first found
    // profile
