...
void BrakeProfile::acceleration_brake(double v0, double a0, double vMax,
  ...
  } else if ((v_at_a_max < vMin && jMax > 0) ||
             (v_at_a_max > vMin && jMax < 0)) {
...
void BrakeProfile::velocity_brake(double v0, double a0, double vMax,
                                  double vMin, double, double aMin,
                                  double jMax) {
  j[0] = -jMax;
  const double t_to_a_min = (a0 - aMin) / jMax;
  const double t_to_v_max =
      a0 / jMax + std::sqrt(a0 * a0 + 2 * jMax * (v0 - vMax)) / std::abs(jMax);
  const double t_to_v_min =
      a0 / jMax + std::sqrt(a0 * a0 / 2 + jMax * (v0 - vMin)) / std::abs(jMax);
  ...
  if (t_to_a_min < t_min_to_v_max) {
    ...
    const double t_to_v_min_with_constant =
        aMin / (2 * jMax) - (v_at_a_min - vMin) / aMin;

    t[0] = std::max(t_to_a_min - eps, 0.0);
    t[1] = std::max(
        std::min(t_to_v_max_with_constant, t_to_v_min_with_constant), 0.0);
...
void BrakeProfile::get_position_brake_trajectory(double v0, double a0,
  ...
  } else if ((v0 > vMax && v_at_a_zero(v0, a0, -jMax) > vMin) ||
             (a0 > 0 && v_at_a_zero(v0, a0, jMax) > vMax)) {
    velocity_brake(v0, a0, vMax, vMin, aMax, aMin, jMax);

  } else if ((v0 < vMin && v_at_a_zero(v0, a0, jMax) < vMax) ||
             (a0 < 0 && v_at_a_zero(v0, a0, -jMax) < vMin)) {
    velocity_brake(v0, a0, vMin, vMax, aMin, aMax, -jMax);
  }
}

void BrakeProfile::get_second_order_position_brake_trajectory(
    double v0, double vMax, double vMin, double aMax, double aMin) {
  t[0] = 0.0;
  t[1] = 0.0;
  j[0] = 0.0;
  j[1] = 0.0;
...
void BrakeProfile::get_velocity_brake_trajectory(double a0, double aMax,
