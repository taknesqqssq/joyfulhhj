...
PositionThirdOrderStep1::PositionThirdOrderStep1(
    double p0, double v0, double a0, double pf, double vf, double af,
    double vMax, double vMin, double aMax, double aMin, double jMax)
    : v0(v0), a0(a0), vf(vf), af(af), _vMax(vMax), _vMin(vMin), _aMax(aMax),
      _aMin(aMin), _jMax(jMax) {
  pd = pf - p0;
...
void PositionThirdOrderStep1::time_all_vel(ProfileIter &profile, double vMax,
  ...
  profile->t[1] =
      (a0_a0 / 2 - aMax * aMax - jMax * (v0 - vMax)) / (aMax * jMax);
  profile->t[2] = aMax / jMax;
  profile->t[3] =
      (3 * (a0_p4 * aMin - af_p4 * aMax) +
       8 * aMax * aMin * (af_p3 - a0_p3 + 3 * jMax * (a0 * v0 - af * vf)) +
       6 * a0_a0 * aMin * (aMax * aMax - 2 * jMax * v0) -
       6 * af_af * aMax * (aMin * aMin - 2 * jMax * vf) -
       12 * jMax *
           (aMax * aMin *
                (aMax * (v0 + vMax) - aMin * (vf + vMax) - 2 * jMax * pd) +
            (aMin - aMax) * jMax * vMax * vMax +
            jMax * (aMax * vf_vf - aMin * v0_v0))) /
      (24 * aMax * aMin * jMax_jMax * vMax);
  profile->t[4] = -aMin / jMax;
  profile->t[5] =
      -(af_af / 2 - aMin * aMin - jMax * (vf - vMax)) / (aMin * jMax);
  profile->t[6] = profile->t[4] + af / jMax;

  if (profile->check<ControlSigns::UDDU, ReachedLimits::ACC0_ACC1_VEL>(
  ...
  profile->t[1] = 0;
  profile->t[2] = t_acc0;
  profile->t[3] = -(3 * af_p4 - 8 * aMin * (af_p3 - a0_p3) -
                    24 * aMin * jMax * (a0 * v0 - af * vf) +
                    6 * af_af * (aMin * aMin - 2 * jMax * vf) -
                    12 * jMax *
                        (2 * aMin * jMax * pd + aMin * aMin * (vf + vMax) +
  ...
  if (profile->check<ControlSigns::UDDU, ReachedLimits::ACC1_VEL>(
          jMax, vMax, vMin, aMax, aMin)) {
    add_profile(profile);
    return;
  }
  ...
  profile->t[1] =
      (a0_a0 / 2 - aMax * aMax - jMax * (v0 - vMax)) / (aMax * jMax);
  profile->t[2] = aMax / jMax;
  profile->t[3] = (3 * a0_p4 + 8 * aMax * (af_p3 - a0_p3) +
                   24 * aMax * jMax * (a0 * v0 - af * vf) +
                   6 * a0_a0 * (aMax * aMax - 2 * jMax * v0) -
                   12 * jMax *
                       (-2 * aMax * jMax * pd + aMax * aMax * (v0 + vMax) +
  ...
  if (profile->check<ControlSigns::UDDU, ReachedLimits::ACC0_VEL>(
          jMax, vMax, vMin, aMax, aMin)) {
    add_profile(profile);
    return;
  }
  ...
  profile->t[3] = (af_p3 - a0_p3) / (3 * jMax_jMax * vMax) +
                  (a0 * v0 - af * vf + (af_af * t_acc1 + a0_a0 * t_acc0) / 2) /
                      (jMax * vMax) -
                  (v0 / vMax + 1.0) * t_acc0 - (vf / vMax + 1.0) * t_acc1 +
                  pd / vMax;
  // profile->t[4] = t_acc1;
  // profile->t[5] = 0;
  // profile->t[6] = t_acc1 + af/jMax;

  if (profile->check<ControlSigns::UDDU, ReachedLimits::VEL>(jMax, vMax, vMin,
                                                             aMax, aMin)) {
    add_profile(profile);
  }
}

void PositionThirdOrderStep1::time_acc0_acc1(ProfileIter &profile, double vMax,
                                             double vMin, double aMax,
                                             double aMin, double jMax,
                                             bool return_after_found) const {
  double h1 =
      (3 * (af_p4 * aMax - a0_p4 * aMin) +
       aMax * aMin *
           (8 * (a0_p3 - af_p3) + 3 * aMax * aMin * (aMax - aMin) +
            6 * aMin * af_af - 6 * aMax * a0_a0) +
       12 * jMax *
           (aMax * aMin * ((aMax - 2 * a0) * v0 - (aMin - 2 * af) * vf) +
            aMin * a0_a0 * v0 - aMax * af_af * vf)) /
  ...
  if (h1 >= 0) {
    h1 = std::sqrt(h1) / 2;
    const double h2 =
        a0_a0 / (2 * aMax * jMax) + (aMin - 2 * aMax) / (2 * jMax) - v0 / aMax;
    const double h3 =
        -af_af / (2 * aMin * jMax) - (aMax - 2 * aMin) / (2 * jMax) + vf / aMin;
    ...
    if (h2 > h1 / aMax && h3 > -h1 / aMin) {
      ...
      if (profile->check<ControlSigns::UDDU, ReachedLimits::ACC0_ACC1, true>(
              jMax, vMax, vMin, aMax, aMin)) {
        add_profile(profile);
...
void PositionThirdOrderStep1::time_all_none_acc0_acc1(
    ProfileIter &profile, double vMax, double vMin, double aMax, double aMin,
    double jMax, bool return_after_found) const {
  // NONE UDDU / UDUD Strategy: t7 == 0 (equals UDDU), this one is in particular
  ...
  polynom_none[2] = 4 * (a0_p3 - af_p3 + 3 * jMax * (af * vf - a0 * v0)) /
                        (3 * jMax * jMax_jMax) -
                    4 * pd / jMax;
  ...
  const double h0_acc0 =
      3 * (af_p4 - a0_p4) + 8 * (a0_p3 - af_p3) * aMax +
      24 * aMax * jMax * (af * vf - a0 * v0) -
      6 * a0_a0 * (aMax * aMax - 2 * jMax * v0) +
      6 * af_af * (aMax * aMax - 2 * jMax * vf) +
      12 * jMax *
          (jMax * (vf_vf - v0_v0 - 2 * aMax * pd) - aMax * aMax * (vf - v0));
  ...
  // ACC1
  const double h3_acc1 =
      -(a0_a0 + af_af) / (2 * jMax * aMin) + aMin / jMax + (vf - v0) / aMin;
  const double t_min_acc1 = (aMin - a0) / jMax;
  const double t_max_acc1 = (aMax - a0) / jMax;

  const double h0_acc1 =
      (a0_p4 - af_p4) / 4 + 2 * (af_p3 - a0_p3) * aMin / 3 +
      (a0_a0 - af_af) * aMin * aMin / 2 +
      jMax * (af_af * vf + a0_a0 * v0 +
              2 * aMin * (jMax * pd - a0 * v0 - af * vf) +
  ...
  polynom_acc1[1] =
      (5 * a0_a0 + aMin * (aMin - 6 * a0) + 2 * jMax * v0) / jMax_jMax;
  ...
  // polynom_none_min[2] += (2 * polynom_none[1] + (3 * polynom_none[0] + 4 *
  // t_min_none) * t_min_none) * t_min_none; polynom_none_min[3] +=
  // (polynom_none[2] + (polynom_none[1] + (polynom_none[0] + t_min_none) *
  // t_min_none) * t_min_none) * t_min_none;
  ...
  polynom_acc0_min[2] += (2 * polynom_acc0[1] +
                          (3 * polynom_acc0[0] + 4 * t_min_acc0) * t_min_acc0) *
                         t_min_acc0;
  polynom_acc0_min[3] +=
      (polynom_acc0[2] +
       (polynom_acc0[1] + (polynom_acc0[0] + t_min_acc0) * t_min_acc0) *
           t_min_acc0) *
      t_min_acc0;

  // const bool polynom_none_has_solution = (polynom_none_min[0] < 0.0) ||
  // (polynom_none_min[1] < 0.0) || (polynom_none_min[2] < 0.0) ||
  // (polynom_none_min[3] <= 0.0);
  const bool polynom_acc0_has_solution =
      (polynom_acc0_min[0] < 0.0) || (polynom_acc0_min[1] < 0.0) ||
      (polynom_acc0_min[2] < 0.0) || (polynom_acc0_min[3] <= 0.0);
  const bool polynom_acc1_has_solution =
      (polynom_acc1[0] < 0.0) || (polynom_acc1[1] < 0.0) ||
      (polynom_acc1[2] < 0.0) || (polynom_acc1[3] <= 0.0);

  auto roots_none = roots::solve_quart_monic(polynom_none);
  auto roots_acc0 = polynom_acc0_has_solution
                        ? roots::solve_quart_monic(polynom_acc0)
                        : roots::PositiveSet<double, 4>{};
  auto roots_acc1 = polynom_acc1_has_solution
                        ? roots::solve_quart_monic(polynom_acc1)
                        : roots::PositiveSet<double, 4>{};
  ...
  for (double t : roots_none) {
    ...
    if (t > DBL_EPSILON) {
      ...
      const double orig =
          -h2_h2 / (4 * jMax * t) + h2_none * (af / jMax + t) +
          (4 * a0_p3 + 2 * af_p3 - 6 * a0_a0 * (af + 2 * jMax * t) +
           12 * (af - a0) * jMax * v0 +
           3 * jMax_jMax * (-4 * pd + (h1 + 8 * v0) * t)) /
              (12 * jMax_jMax);
      const double deriv =
          h2_none + 2 * v0 - a0_a0 / jMax + h2_h2 / (4 * h1) + (3 * h1) / 4;
    ...
    if (profile->check<ControlSigns::UDDU, ReachedLimits::NONE>(
            jMax, vMax, vMin, aMax, aMin)) {
      add_profile(profile);
      if (return_after_found) {
        return;
  ...
  for (double t : roots_acc0) {
    ...
    if (t > DBL_EPSILON) {
      ...
      const double orig =
          h0_acc0 / (12 * jMax_jMax * t) + t * (h2_acc0 + h1 * (h1 - 2 * aMax));
    ...
    if (profile->check<ControlSigns::UDDU, ReachedLimits::ACC0>(
            jMax, vMax, vMin, aMax, aMin)) {
      add_profile(profile);
      if (return_after_found) {
        return;
  ...
  for (double t : roots_acc1) {
    ...
    if (t > DBL_EPSILON) {
      ...
      double orig =
          -(h0_acc1 / 2 +
            h1 * (h5 + a0 * (aMin - 2 * h1) * (aMin - h1) +
                  a0_a0 * (5 * h1 / 2 - 2 * aMin) + aMin * aMin * h1 / 2 +
                  jMax * (h1 / 2 - aMin) * (h1 * t + 2 * v0))) /
          jMax;
      double deriv =
          (aMin - a0 - h1) * (h2_acc1 + h1 * (4 * a0 - aMin + 2 * h1));
    ...
    if (profile->check<ControlSigns::UDDU, ReachedLimits::ACC1, true>(
            jMax, vMax, vMin, aMax, aMin)) {
      add_profile(profile);
      if (return_after_found) {
        return;
...
void PositionThirdOrderStep1::time_acc1_vel_two_step(ProfileIter &profile,
  ...
  profile->t[4] = -aMin / jMax;
  profile->t[5] =
      -(af_af / 2 - aMin * aMin + jMax * (vMax - vf)) / (aMin * jMax);
  profile->t[6] = profile->t[4] + af / jMax;
...
void PositionThirdOrderStep1::time_acc0_two_step(ProfileIter &profile,
                                                 double vMax, double vMin,
                                                 double aMax, double aMin,
                                                 double jMax) const {
  // Two step
  ...
  {
    profile->t[0] = (-a0 + aMax) / jMax;
    profile->t[1] = (a0_a0 + af_af - 2 * aMax * aMax + 2 * jMax * (vf - v0)) /
                    (2 * aMax * jMax);
  ...
  // Three step - Removed aMax
  {
    ...
    const double h2 = a0_p3 + 2 * af_p3 + 6 * jMax_jMax * pd +
                      6 * (af - a0) * jMax * vf - 3 * a0 * af_af;
    const double h1 =
        std::sqrt(
            2 *
            (2 * h2 * h2 +
             h0 * (a0_p4 - 6 * a0_a0 * (af_af + 2 * jMax * vf) +
                   8 * a0 * (af_p3 + 3 * jMax_jMax * pd + 3 * af * jMax * vf) -
                   3 * (af_p4 + 4 * af_af * jMax * vf +
                        4 * jMax_jMax * (vf_vf - v0_v0))))) *
        std::abs(jMax) / jMax;
    profile->t[0] = (4 * af_p3 + 2 * a0_p3 - 6 * a0 * af_af +
                     12 * jMax_jMax * pd + 12 * (af - a0) * jMax * vf + h1) /
                    (2 * jMax * h0);
    profile->t[1] = -h1 / (jMax * h0);
    profile->t[2] = (-4 * a0_p3 - 2 * af_p3 + 6 * a0_a0 * af +
                     12 * jMax_jMax * pd - 12 * (af - a0) * jMax * v0 + h1) /
                    (2 * jMax * h0);
    profile->t[3] = 0;
    profile->t[4] = 0;
    profile->t[5] = 0;
  ...
  // Three step - t=(aMax - aMin)/jMax
  {
    ...
    profile->t[0] = (-a0 + aMax) / jMax;
    profile->t[1] = (a0_a0 - af_af) / (2 * aMax * jMax) +
                    (vf - v0 + jMax * t * t) / aMax - 2 * t;
    profile->t[2] = t;
    profile->t[3] = 0;
    profile->t[4] = 0;
    profile->t[5] = 0;
...
void PositionThirdOrderStep1::time_vel_two_step(ProfileIter &profile,
                                                double vMax, double vMin,
                                                double aMax, double aMin,
                                                double jMax) const {
...
void PositionThirdOrderStep1::time_none_two_step(ProfileIter &profile,
                                                 double vMax, double vMin,
                                                 double aMax, double aMin,
                                                 double jMax) const {
  // Two step
  {
    const double h0 = std::sqrt((a0_a0 + af_af) / 2 + jMax * (vf - v0)) *
                      std::abs(jMax) / jMax;
...
bool PositionThirdOrderStep1::time_all_single_step(Profile *profile,
  ...
  if (std::abs(a0) > DBL_EPSILON) {
    ...
    profile->t[3] = (-v0 + q) / a0;
    if (profile->t[3] >= 0.0 &&
        profile->check<ControlSigns::UDDU, ReachedLimits::NONE>(0.0, vMax, vMin,
                                                                aMax, aMin)) {
      return true;
    }
    ...
    profile->t[3] = pd / v0;
    if (profile->check<ControlSigns::UDDU, ReachedLimits::NONE>(0.0, vMax, vMin,
                                                                aMax, aMin)) {
      return true;
    }
...
bool PositionThirdOrderStep1::get_profile(const Profile &input, Block &block) {
  ...
  if (_jMax == 0.0 || _aMax == 0.0 || _aMin == 0.0) {
    ...
    if (time_all_single_step(&p, _vMax, _vMin, _aMax, _aMin, _jMax)) {
      ...
      if (std::abs(v0) > DBL_EPSILON || std::abs(a0) > DBL_EPSILON) {
        block.a = Block::Interval(block.t_min,
  ...
  if (std::abs(vf) < DBL_EPSILON && std::abs(af) < DBL_EPSILON) {
    ...
    if (std::abs(v0) < DBL_EPSILON && std::abs(a0) < DBL_EPSILON &&
        std::abs(pd) < DBL_EPSILON) {
      time_all_none_acc0_acc1(profile, vMax, vMin, aMax, aMin, jMax, true);

    } else {
      // There is no blocked interval when vf==0 && af==0, so return after first
      // found profile
...
return_block:
  return Block::calculate_block(block, valid_profiles,
                                std::distance(start, profile));
}
