...
VelocityThirdOrderStep1::VelocityThirdOrderStep1(double v0, double a0,
                                                 double vf, double af,
                                                 double aMax, double aMin,
                                                 double jMax)
    : a0(a0), af(af), _aMax(aMax), _aMin(aMin), _jMax(jMax) {
  vd = vf - v0;
}

void VelocityThirdOrderStep1::time_acc0(ProfileIter &profile, double aMax,
                                        double aMin, double jMax, bool) const {
  profile->t[0] = (-a0 + aMax) / jMax;
  profile->t[1] =
      (a0 * a0 + af * af) / (2 * aMax * jMax) - aMax / jMax + vd / aMax;
  ...
  if (profile->check_for_velocity<ControlSigns::UDDU, ReachedLimits::ACC0>(
          jMax, aMax, aMin)) {
    add_profile(profile);
  }
}

void VelocityThirdOrderStep1::time_none(ProfileIter &profile, double aMax,
  ...
  if (h1 >= 0.0) {
    ...
    // Solution 1
    {
      ...
      if (profile->check_for_velocity<ControlSigns::UDDU, ReachedLimits::NONE>(
              jMax, aMax, aMin)) {
        add_profile(profile);
...
bool VelocityThirdOrderStep1::time_all_single_step(Profile *profile,
                                                   double aMax, double aMin,
                                                   double jMax) const {
...
bool VelocityThirdOrderStep1::get_profile(const Profile &input, Block &block) {
  ...
  if (std::abs(af) < DBL_EPSILON) {
    // There is no blocked interval when af==0, so return after first found
    // profile
