...
VelocitySecondOrderStep1::VelocitySecondOrderStep1(double v0, double vf,
                                                   double aMax, double aMin)
    : _aMax(aMax), _aMin(aMin) {
  vd = vf - v0;
}
...
bool VelocitySecondOrderStep1::get_profile(const Profile &input, Block &block) {
  ...
  if (p.check_for_second_order_velocity<Profile::ControlSigns::UDDU,
                                        Profile::ReachedLimits::ACC0>(af)) {
