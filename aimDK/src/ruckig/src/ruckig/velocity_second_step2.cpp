...
VelocitySecondOrderStep2::VelocitySecondOrderStep2(double tf, double v0,
                                                   double vf, double aMax,
                                                   double aMin)
    : tf(tf), _aMax(aMax), _aMin(aMin) {
  vd = vf - v0;
}
...
bool VelocitySecondOrderStep2::get_profile(Profile &profile) {
  ...
  if (profile.check_for_second_order_velocity_with_timing<
          Profile::ControlSigns::UDDU, Profile::ReachedLimits::NONE>(
          tf, af, _aMax, _aMin)) {
