...
nlohmann::json CloudClient::post(const nlohmann::json &params,
                                 bool throw_error) {
  ...
  if (res->status != 200) {
    if (throw_error) {
      throw RuckigError("could not reach cloud API server, error code: " +
                        std::to_string(res->status) + " " + res->body);
    } else {
      std::cout << "[ruckig] could not reach cloud API server, error code: "
